...
{  int i, tmp; text_t *txt1, *txt2; char *c;
   ...
   txt1 = create_text();
   txt2 = create_text();
   printf("Leght f Text 1:%d\n",length_text(txt1));
   printf("Leght f Text 2:%d\n",length_text(txt2));
   ...
   text_t *txt3 = create_text();
   printf("found at the n+1 txt3 line:   %s\n",get_line(txt3,  1));

   char *ch = (char*)malloc(sizeof(char));
   for(i=1;i<=100;i++){
	sprintf(ch,"%d",i);
	insert_line(txt3,i,ch);
   }	
   printf("Success 1\n")	;
   for(i=101;i<=125;i++){
	sprintf(ch,"%d",i);
   ...
   for(i=125;i>100;i--){
	printf("Deleted Line %d: %s\n",i,delete_line(txt3,i));
   }	
   printf("Success 3\n")	;
   	

   printf("lenght txt3 line:   %d\n",length_text(txt3));
